...
// Added definition outside
	...
	FeetInches& operator+= (const FeetInches &); // operator +=
...
std::ostream &operator << (std::ostream &strm, const FeetInches &rhs) {
    strm << rhs.feet << "\'" << rhs.inches << "\" "; 
    return strm;
}

std::istream &operator >> (std::istream &strm, FeetInches &rhs) {
    char c; // 3'5"
    strm >> rhs.feet;
    strm >> c;
    strm >> rhs.inches;
...
FeetInches::FeetInches(std::string s) {
